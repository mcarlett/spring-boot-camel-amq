...
This quickstart requires AMQ Broker to have been deployed and running first. To install AMQ Broker into OpenShift or Kubernetes follow https://access.redhat.com/documentation/en-us/red_hat_amq_broker/7.10/html/deploying_amq_broker_on_openshift/deploying-broker-on-ocp-using-operator_broker-ocp[documentation].
...
. Make sure AMQ Broker Operator is installed in the cluster or in the namespace
...
. Before running the quickstart, you need to create and configure AMQ Broker.
Create a `secret` named `my-tls-secret` containing TLS certificates to use to connect as described in https://access.redhat.com/documentation/en-us/red_hat_amq_broker/7.10/html/deploying_amq_broker_on_openshift/assembly-br-configuring-operator-based-deployments_broker-ocp#proc-br-configuring-one-way-tls_broker-ocp[configuring-one-way-tls_broker-ocp].
Run the following commands to apply configuration files:

+
[source,bash,options="nowrap",subs="attributes+"]
----
$ keytool -genkey -alias broker -keyalg RSA -keystore ./broker.ks --storepass changeit -dname "CN=broker, OU=Unknown, O=Unknown, L=Unknown, ST=Unknown, C=Unknown"
$ keytool -export -alias broker -keystore ./broker.ks -file ./broker_cert.pem --storepass changeit
$ keytool -import -alias broker -keystore ./client.ts -file ./broker_cert.pem --storepass changeit --noprompt
...
. Make sure the AMQ Broker is ready
+
[source,bash,options="nowrap",subs="attributes+"]
----
$ oc get pod -l ActiveMQArtemis=spring-boot-camel-amq
----
